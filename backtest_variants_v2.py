"""
XAUUSD Strategy Backtest V2 - Filter-Lockerung basierend auf Diagnose

Diagnose-Top-3-Bottlenecks: Cooldown, OB Midpoint, RSI.
Hier werden 7 Varianten verglichen, die genau diese drei Gates lockern.
Alle anderen Parameter bleiben identisch zu V6 Live:
  chop_filter=False, entry_confirmation=False, structural_sl_tp=False,
  SCORE_THRESHOLD=6.0, Session London 7 - NY 21 UTC, no weekend,
  no Friday >= 19 UTC (FTMO gap rule).

Datenladeroutine, Indikatoren, Backtest-Engine und Metriken sind 1:1 uebernommen
aus backtest_variants.py.

USAGE (Railway Shell):
python backtest_variants_v2.py

TWELVE_DATA_KEY wird aus den Railway Environment Variables gelesen.
...
# ==============================
# SIGNAL GENERATOR (config-parameterisiert)
# ==============================
...
# ==============================
# BACKTEST ENGINE (aus backtest_variants.py, erweitert um Weekend + Friday-Stop)
# ==============================
...
# ==============================
# METRICS (erweitert um trades_per_day)
# ==============================
...
def print_sniper_recommendation(results, ranked_by_exp):
    print('=' * 90)
    print('SNIPER RECOMMENDATION')
    print('=' * 90)
    print('Kriterien: trades_per_day 0.5-3.0, winrate >= 55%, expectancy > 0')
    print()

    def passes(m):
        return (0.5 <= m['trades_per_day'] <= 3.0
                and m['winrate'] >= 55.0
                and m['expectancy'] > 0)
    ...
    if matches:
        matches_sorted = sorted(matches, key=lambda x: x[1]['expectancy'], reverse=True)
        print(f'Varianten im Sniper-Ziel-Fenster: {len(matches_sorted)}')
        for rank, (name, m) in enumerate(matches_sorted, 1):
            print(f'  {rank}. {name}: Exp ${m["expectancy"]} | WR {m["winrate"]}% | '
                  f'T/Day {m["trades_per_day"]} | PnL ${m["total_pnl"]}')
    else:
        print('Keine Variante im Sniper-Ziel-Fenster')
        best_name, best_m = ranked_by_exp[0]
        print(f'Beste nach Expectancy: {best_name} (Exp ${best_m["expectancy"]})')
        missing = []
        if not (0.5 <= best_m['trades_per_day'] <= 3.0):
            missing.append(f'trades_per_day={best_m["trades_per_day"]} (Ziel 0.5-3.0)')
...
def print_change_vs_baseline(results):
    ...
    for name, m in results.items():
        if name == 'V_BASELINE':
            continue
        diff_trades = m['total_trades'] - b['total_trades']
        pct_trades = (diff_trades / b['total_trades'] * 100) if b['total_trades'] else 0.0
        print(f'{name:18s} {diff_trades:+d} trades ({pct_trades:+.1f}%), '
              f'WR {m["winrate"]:5.1f}% (baseline {b["winrate"]:5.1f}%), '
              f'Exp ${m["expectancy"]:6.2f} (baseline ${b["expectancy"]:6.2f})')
    print()
...
def main():
    print('=' * 90)
    print('XAUUSD STRATEGY BACKTEST V2 - FILTER LOCKERUNG (7 VARIANTEN)')
    print('=' * 90)
    print(f'Score threshold: {SCORE_THRESHOLD}')
    print(f'Fixed: chop_filter=False, entry_confirmation=False, structural_sl_tp=False')
    print(f'Session: London {LONDON_OPEN_UTC} - NY {NY_CLOSE_UTC} UTC, '
          f'no weekend, no Friday >= {FRIDAY_STOP_UTC} UTC')
    print()
    ...
    for variant_name, config in VARIANTS.items():
        print(f'Running {variant_name} (cooldown {config["cooldown_win"]}/{config["cooldown_loss"]}, '
              ...
              f'midpoint={config["ob_midpoint"]})...', flush=True)
    ...
    ranked_exp = print_ranking_expectancy(results)
    print_ranking_pnl(results)
    print_sniper_recommendation(results, ranked_exp)
