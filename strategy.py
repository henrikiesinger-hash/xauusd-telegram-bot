...
def generate_signal(data_m5, candle_index=0):
    ...
    log.info(
        "Price:%.2f Dir:%s Score:%.1f | %s",
        price, direction, score, " | ".join(parts)
    )
    ...
    return {
        ...
        "sl_dist": sl_dist,
        "score": score,
        "confidence": "SNIPER" if score >= 8.5 else "HIGH" if score >= 7.0 else "MODERATE",
        "notes": " | ".join(parts),
    }